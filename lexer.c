...
char *extract(const char *begin,const char *end){
	char *result = safeAlloc(end - begin + 1);
		...
		switch(*pch){
			...
			default:
				if(isalpha(*pch)||*pch=='_'){
					...
					short isDouble = 0;
					for (start = pch++; *pch == '.' || *pch == 'e' || *pch == 'E'|| *pch == '-' || *pch == '+'|| isdigit(*pch); pch++)
					{
						if (*pch == '.') //sa ai si cifre dupa astea.
						{
							if(!isdigit(pch[1])){
								printf("not a valid number at line %d", tk->line);
								exit(-1);
							}
							else{
								isDouble = 1;
